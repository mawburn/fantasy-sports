...
class ContestRecommendationRequest(BaseModel):
    ...
    min_fun_score: float = Field(default=60.0, ge=0, le=100, description="Minimum fun score threshold")
    min_expected_value: float = Field(default=-0.10, description="Minimum expected value (negative allows some -EV for fun)")
    max_field_size: int = Field(default=100000, gt=0, description="Maximum contest field size")
    max_contests: int = Field(default=10, gt=0, le=50, description="Maximum number of contest recommendations")
    
    # Skill assessment
    skill_edge: float = Field(default=0.0, ge=-0.5, le=0.5, description="Estimated skill edge over field (-0.5 to 0.5)")
...
async def recommend_contests(
    request: ContestRecommendationRequest,
    ...
    try:
        ...
        for contest in recommendations:
            ...
            response_contests.append(ContestRecommendationResponse(
        ...
        logger.error(f"Error generating contest recommendations: {e}")
...
async def analyze_contest(
    ...
    try:
        ...
        logger.error(f"Error analyzing contest {contest_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to analyze contest: {str(e)}")
...
async def get_selection_summary(
    ...
    try:
        ...
        avg_fun_score = sum(c.fun_score for c in recommendations) / len(recommendations) if recommendations else 0
