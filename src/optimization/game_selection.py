...
class ContestAnalyzer:
    ...
    def _classify_contest_type(self, contest: DraftKingsContest) -> ContestType:
        ...
        if any(term in name for term in ["50/50", "double up", "head to head"]):
            ...
            payout_percentage = (
                contest.total_prizes / (contest.entry_fee * contest.entries)
    ...
    def _analyze_payout_structure(self, contest: DraftKingsContest) -> dict[str, float]:
        ...
        rake_percentage = (
            1 - (contest.total_prizes / total_entry_fees) if total_entry_fees > 0 else 0
        )
        ...
        if contest_type == ContestType.CASH:
            ...
            avg_payout = (
                contest.total_prizes / (contest.entries * win_probability)
                if contest.entries > 0
                else 0
            )
    ...
    def _analyze_field_competitiveness(self, contest: DraftKingsContest) -> dict[str, float]:
        ...
        total_fees = (
            contest.entry_fee * contest.entries if contest.entries > 0 else contest.entry_fee
        )
        overlay = (contest.total_prizes - total_fees) / total_fees if total_fees > 0 else 0

        return {"skill_level": skill_level, "overlay": overlay}

    def _calculate_risk_metrics(
        self, contest: DraftKingsContest, payout_metrics: dict[str, float]
    ) -> dict[str, float]:
        ...
        if contest_type == ContestType.CASH:
            ...
            avg_win = (
                contest.total_prizes / (contest.entries * win_probability)
                if contest.entries > 0 and win_probability > 0
                else 0
            )
            variance = (
                win_probability * (avg_win - contest.entry_fee) ** 2
                + (1 - win_probability) * (contest.entry_fee) ** 2
            )
        else:
        ...
        downside_risk = 1 - win_probability if contest_type == ContestType.CASH else 0.8
        ...
        if win_probability > 0 and expected_value > 0:
            ...
            if avg_payout > contest.entry_fee:
                kelly_fraction = (
                    win_probability * avg_payout - (1 - win_probability) * contest.entry_fee
                ) / avg_payout
...
class RiskAssessment:
    ...
    def assess_portfolio_risk(
        self, selected_contests: list[ContestMetrics], bankroll: float
    ) -> dict[str, float]:
            ...
            for i in range(len(individual_variances)):
                for j in range(i + 1, len(individual_variances)):
                    correlation_terms += (
                        2
                        * correlation_factor
                        * math.sqrt(individual_variances[i] * individual_variances[j])
                    )
        ...
        portfolio_kelly = min(0.25, total_ev / (bankroll * 0.1)) if total_ev > 0 else 0.0
    ...
    def is_within_risk_tolerance(
        self, portfolio_risk: dict[str, float], settings: GameSelectionSettings
    ) -> bool:
...
class GameSelectionEngine:
    ...
    def recommend_contests(
        self, settings: GameSelectionSettings, skill_edge: float = 0.0
    ) -> list[ContestMetrics]:
    ...
    def _passes_user_filters(
        self, metrics: ContestMetrics, settings: GameSelectionSettings
    ) -> bool:
        ...
        return not (settings.avoid_types and metrics.contest_type in settings.avoid_types)

    def _score_contests(
        self, contests: list[ContestMetrics], settings: GameSelectionSettings
    ) -> list[ContestMetrics]:
    ...
    def _select_optimal_portfolio(
        self, scored_contests: list[ContestMetrics], settings: GameSelectionSettings
    ) -> list[ContestMetrics]:
        ...
        for contest in scored_contests:
            ...
            portfolio_risk = self.risk_assessor.assess_portfolio_risk(
                potential_portfolio, settings.bankroll
            )
