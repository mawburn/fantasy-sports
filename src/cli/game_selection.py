...
def recommend_contests(
    ...
    max_entry_fee: Optional[float] = typer.Option(
        None, 
        help="Maximum entry fee per contest (default: 10% of bankroll)"
    ),
    max_total_risk: Optional[float] = typer.Option(
        None, 
        help="Maximum total risk (default: 10% of bankroll)"
    ),
    min_fun_score: float = typer.Option(
        60.0, 
        help="Minimum fun score (0-100)"
    ),
    min_expected_value: float = typer.Option(
        -0.10, 
        help="Minimum expected value (negative allows some -EV for fun)"
    ),
    max_contests: int = typer.Option(
        10, 
        help="Maximum number of contest recommendations"
    ),
    skill_edge: float = typer.Option(
        0.0, 
        help="Your estimated skill edge over field (-0.5 to 0.5)"
    ),
    ...
    try:
        ...
        console.print(f"🎯 Analyzing contests for {risk_tolerance} player with ${bankroll:,.0f} bankroll...")
        ...
        if total_cost / bankroll > 0.15:
            console.print(f"\n⚠️  High bankroll risk detected! Consider reducing total exposure.", style="yellow")
...
def analyze_contest(
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            contest = db.query(DraftKingsContest).filter(
                DraftKingsContest.contest_id == contest_id
            ).first()
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            if recommendations:
                ...
                console.print(f"   🎉 Average Fun Score: {avg_fun:.1f}/100")
                console.print(f"   🎲 Portfolio Risk: {(total_entry_fees/bankroll)*100:.1f}% of bankroll")
            else:
...
def _display_table_output(recommendations: List, settings: GameSelectionSettings) -> None:
    ...
    for contest in recommendations:
        ...
        name = contest.contest_name[:25] + "..." if len(contest.contest_name) > 28 else contest.contest_name
...
def _display_contest_table(metrics, skill_edge: float) -> None:
    ...
    # Key metrics
    fun_assessment = "Excellent" if metrics.fun_score >= 80 else "Good" if metrics.fun_score >= 60 else "Fair"
    table.add_row("Fun Score", f"{metrics.fun_score:.1f}/100", fun_assessment)
    
    ev_assessment = "Positive Edge" if metrics.expected_value > 0 else "Break Even" if metrics.expected_value > -0.1 else "Negative"
    ev_color = "bright_green" if metrics.expected_value >= 0 else "red"
    table.add_row("Expected Value", f"[{ev_color}]${metrics.expected_value:+.2f}[/{ev_color}]", ev_assessment)
    ...
    risk_assessment = "Low" if metrics.downside_risk < 0.5 else "Medium" if metrics.downside_risk < 0.8 else "High"
    table.add_row("Downside Risk", f"{metrics.downside_risk*100:.0f}%", risk_assessment)
    
    if skill_edge != 0:
        table.add_row("", "", "")  # Separator
        table.add_row("Skill Edge Applied", f"{skill_edge:+.1%}", f"{'Advantage' if skill_edge > 0 else 'Disadvantage'}")
