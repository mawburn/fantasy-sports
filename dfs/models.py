...
try:
    ...
    logger.warning(
        "Hyperparameter manager not available. Using default hyperparameters."
    )
...
def get_optimal_device():
    ...
    if torch.backends.mps.is_available():
        ...
        logger.info(
            f"Using MPS (Metal) acceleration on Apple Silicon with {cpu_count} CPU threads"
        )
        return device
...
if OPTIMAL_DEVICE.type == "mps":
    ...
    logger.warning(
        "MPS fallback enabled - some operations may run on CPU instead of GPU, "
...
class LRFinder:
    ...
    def _find_optimal_lr(self) -> float:
        ...
        logger.info(
            f"Found optimal LR: {optimal_lr:.2e} (steepest decline at index {min_gradient_idx})"
        )
    ...
    def plot_results(self, save_path: str = None):
        ...
        try:
            ...
            ax1.semilogx(
                self.learning_rates, self.smoothed_losses, label="Smoothed Loss"
            )
...
class BatchSizeOptimizer:
    ...
    def optimize_batch_size(
        ...
        if not feasible_batch_sizes:
            logger.warning(
                f"No feasible batch sizes found, using minimum: {min(batch_sizes)}"
            )
        ...
        for batch_size in feasible_batch_sizes:
            ...
            try:
                ...
                result = model_trainer._train_epochs(
                    test_train_loader, test_val_loader, epochs_per_test
                )
                ...
                logger.info(
                    f"Batch size {batch_size}: val_loss={result['val_loss']:.4f}"
                )
        ...
        logger.info(
            f"Optimal batch size: {best_batch_size} (val_loss={results[best_batch_size]:.4f})"
        )
...
try:
    ...
    class HyperparameterTuner:
        ...
        def objective(self, trial: Trial) -> float:
            ...
            if "choices" in batch_config:
                batch_size = trial.suggest_categorical(
                    "batch_size", batch_config["choices"]
                )
            ...
            if "choices" in hidden_config:
                hidden_size = trial.suggest_categorical(
                    "hidden_size", hidden_config["choices"]
                )
            ...
            if position == "DST":
                ...
                if HAS_CATBOOST:
                    ...
                    if spearman_corr is None or np.isnan(spearman_corr):
                        logger.warning(
                            f"NaN detected in Spearman correlation for {position} trial {trial.number}"
                        )
                        spearman_corr = 0.0
                    if np.isnan(ndcg_score):
                        logger.warning(
                            f"NaN detected in NDCG@20 score for {position} trial {trial.number}"
                        )
                        ndcg_score = 0.0
                    if np.isnan(r2):
                        logger.warning(
                            f"NaN detected in R² score for {position} trial {trial.number}"
                        )
                        r2 = 0.0
                    if np.isnan(mae):
                        logger.warning(
                            f"NaN detected in MAE for {position} trial {trial.number}"
                        )
        ...
        def optimize(self, n_trials: int = 20, timeout: int = 3600) -> Dict[str, Any]:
            ...
            # Optimize
            logger.info(
                f"Starting hyperparameter optimization with {n_trials} trials (optimizing NDCG@20)..."
            )
            study.optimize(
                self.objective,
                n_trials=n_trials,
...
class HyperparameterValidator:
    ...
    def ab_test(
        ...
        improvements = {
            "r2_improvement": (
                (optimized_stats["mean_r2"] - baseline_stats["mean_r2"])
                / max(abs(baseline_stats["mean_r2"]), 0.001)
            )
            * 100,
            "mae_improvement": (
                (baseline_stats["mean_mae"] - optimized_stats["mean_mae"])
                / max(baseline_stats["mean_mae"], 0.001)
            )
            * 100,
            "rmse_improvement": (
                (baseline_stats["mean_rmse"] - optimized_stats["mean_rmse"])
                / max(baseline_stats["mean_rmse"], 0.001)
            )
            * 100,
            "training_time_change": (
                (optimized_stats["mean_time"] - baseline_stats["mean_time"])
                / max(baseline_stats["mean_time"], 0.001)
            )
            * 100,
        }
    ...
    def _make_decision(self, improvements: Dict[str, float]) -> str:
        ...
        if improvements["r2_improvement"] > 10:
            ...
            improvements["r2_improvement"] > 0
            and improvements["training_time_change"] < 50
        ):
    ...
    def _print_results(self, results: Dict[str, Any]):
        ...
        print(
            f"  Training Time: {baseline['mean_time']:.1f}s ± {baseline['std_time']:.1f}s"
        )
        ...
        print(
            f"  Training Time: {optimized['mean_time']:.1f}s ± {optimized['std_time']:.1f}s"
        )
    ...
    def cross_validate_hyperparameters(
        ...
        logger.info(
            f"Starting {cv_folds}-fold cross-validation for {self.model_config.position}"
        )
        ...
        logger.info(
            f"Cross-validation complete: R²={summary['mean_r2']:.4f}±{summary['std_r2']:.4f}"
        )
...
class CorrelationFeatureExtractor:
    ...
    def extract_qb_correlation_features(
        ...
        with self._get_connection() as conn:
            ...
            """
            teammate_result = conn.execute(
                teammate_query, (team_id, start_date, game_date)
            ).fetchone()
            ...
            stacking_query = """
                ...
                LIMIT 3
            """
            stacking_results = conn.execute(
                stacking_query, (team_id, team_id, start_date, game_date, player_id)
            ).fetchall()
    ...
    def extract_rb_correlation_features(
        ...
        with self._get_connection() as conn:
            ...
            """
            workload_result = conn.execute(
                workload_query, (player_id, start_date, game_date)
            ).fetchone()
    ...
    def extract_wr_te_correlation_features(
        ...
        with self._get_connection() as conn:
            ...
            target_comp_query = """
                ...
                AND ps.targets > 0
            """
            target_comp_result = conn.execute(
                target_comp_query, (team_id, start_date, game_date)
            ).fetchone()
                ...
                features.update(
                    {
                        ...
                        "target_concentration": (target_comp_result[2] or 0)
                        / max(target_comp_result[1] or 1, 1),
                        "wr_target_dominance": (target_comp_result[3] or 0)
                        / max(target_comp_result[1] or 1, 1),
                        "te_target_share": (target_comp_result[4] or 0)
                        / max(target_comp_result[1] or 1, 1),
                    }
                )
    ...
    def extract_correlation_features(
        self, player_id: int, game_id: int, position: str
    ) -> Dict[str, float]:
...
class BaseNeuralModel(ABC):
    ...
    def __init__(self, config: ModelConfig):
        ...
        if HAS_HYPERPARAMETER_MANAGER:
            hyperparams = get_hyperparameter_manager().get_hyperparameters(
                config.position
            )
    ...
    def _normalize_targets(self, y: np.ndarray) -> np.ndarray:
        ...
        if self.y_std == 0:
            raise ValueError(
                f"Target standard deviation is zero for {self.config.position}"
            )

        y_normalized = (y - self.y_mean) / self.y_std
        logger.info(
            f"Normalized {self.config.position} targets: mean={self.y_mean:.2f}, std={self.y_std:.2f}"
        )
        return y_normalized
    ...
    def find_optimal_lr(
        ...
        logger.info(
            f"Found optimal learning rate for {self.config.position}: {optimal_lr:.2e}"
        )
        return optimal_lr
    ...
    def optimize_batch_size(
        ...
        logger.info(
            f"Found optimal batch size for {self.config.position}: {optimal_batch_size}"
        )
    ...
    def tune_hyperparameters(
        ...
        tuner = HyperparameterTuner(
            self.__class__, self.config, X_train, y_train, X_val, y_val, epochs=epochs
        )
        ...
        if hasattr(tuner, "best_model") and tuner.best_model is not None:
            ...
            self._last_validation_spearman = (
        ...
        logger.info(
            f"Applied best hyperparameters for {self.config.position}: {best_params}"
        )
        ...
        validation_mae = best_metrics.get(
            "mae", getattr(self, "_last_validation_mae", None)
        )
        validation_r2 = best_metrics.get(
            "r2", getattr(self, "_last_validation_r2", None)
        )
        validation_spearman = best_metrics.get(
            "spearman", getattr(self, "_last_validation_spearman", None)
        )
        validation_ndcg = best_metrics.get(
            "ndcg_at_k", getattr(self, "_last_validation_ndcg", None)
        )
        logger.info(
            f"Extracted metrics - MAE: {validation_mae}, R²: {validation_r2}, Spearman: {validation_spearman}, NDCG: {validation_ndcg}"
        )
    ...
    def validate_predictions(
        ...
        logger.info(
            f"{position} predictions validated successfully: "
    ...
    def predict(self, X: np.ndarray, validate: bool = True) -> PredictionResult:
        ...
        uncertainty = (
            ...
            else np.std(point_estimate) * 0.5
        )
    ...
    def load_model(self, path: str, input_size: int = None):
        ...
        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            ...
            if input_size is None:
                ...
                if "weight" in first_layer_key:
                    ...
                    raise ValueError(
                        "Cannot determine input size from old model format"
                    )
        ...
        self.is_trained = True
        logger.info(
            f"Model loaded from {path} with input_size={input_size} to device: {self.device}"
        )
...
class RBNeuralModel(BaseNeuralModel):
    ...
    def train_rb_model(self, X_train, y_train, X_val, y_val, salaries_train=None):
        ...
        if (
            len(X_train.shape) > 1 and X_train.shape[1] > 10
        ):  # Assume week is in features
    ...
    def train_wr_model(self, X_train, y_train, X_val, y_val, salaries_train=None):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                assert np.percentile(val_preds_np, 90) > 15, (
                    "No WR ceiling players projected"
                )
...
class TENeuralModel(BaseNeuralModel):
    ...
    def train_te_model(self, X_train, y_train, X_val, y_val, salaries_train=None):
        ...
        if y_train.std() < 1.0:
            logger.warning(
                f"TE model has very low target variance: {y_train.std():.3f}"
            )
        ...
        def te_loss_fn(predictions, targets):
            ...
            ceiling_penalty = (
                F.relu(4 - batch_mean) * 0.5
            )  # Penalize if avg prediction too low

            # Variance preservation - critical for TE position
            batch_std = torch.std(mean_pred)
            variance_penalty = (
                F.relu(2 - batch_std) * 0.8
            )  # TEs need at least 2-point std dev
            ...
            td_regularization = torch.mean(
                torch.abs(estimated_tds - torch.round(estimated_tds)) * 0.1
            )
        ...
        try:
            ...
            with torch.no_grad():
                ...
                logger.info(
                    f"TE Validation - Mean: {pred_mean:.3f}, Std: {pred_std:.3f}, "
                ...
                assert not np.allclose(val_preds_np, val_preds_np[0]), (
                    "All predictions identical - model broken"
                )

                # Check for reasonable score distribution
                high_score_count = np.sum(val_preds_np > 12)
                assert high_score_count > len(val_preds_np) * 0.1, (
                    f"Too few high predictions: {high_score_count}/{len(val_preds_np)}"
                )
...
class DEFCatBoostModel:
    ...
    def __init__(self, config: ModelConfig):
        ...
        if HAS_HYPERPARAMETER_MANAGER:
            ...
            if stored_params:
                logger.info("Loading DST hyperparameters from YAML")
                self.best_params = {
                    ...
                    "bagging_temperature": stored_params.get(
                    ...
                    "min_data_in_leaf": stored_params.get(
                        "catboost_min_data_in_leaf", 3
                    ),
                }
    ...
    def train(
        ...
        train_r2 = (
            1 - np.sum((y - train_pred) ** 2) / train_ss_tot
            if train_ss_tot > 0
            else 0.0
        )
        val_r2 = (
    ...
    def train_component_models(
        ...
        sacks_model.fit(
            X, y_components["sacks"], eval_set=(X_val, y_val_components["sacks"])
        )
        ...
        td_model.fit(X, y_components["td"], eval_set=(X_val, y_val_components["td"]))
    ...
    def predict_components(self, X: np.ndarray) -> dict:
        ...
        if not hasattr(self, "component_models") or not self.component_models:
            raise ValueError(
                "Component models must be trained before making component predictions"
            )
        ...
        td_prob = self.component_models["td"].predict_proba(X)[
            :, 1
        ]  # Probability of TD
    ...
    def tune_hyperparameters(
        ...
        """
        logger.info(
            f"Starting CatBoost hyperparameter optimization for DST ({n_trials} trials)"
        )
        ...
        def objective(trial):
            ...
            # Log trial progress
            logger.info(
                f"Trial {trial.number + 1}: MAE={val_mae:.4f}, {metric_name}={metric_value:.4f}"
            )
        ...
        study = optuna.create_study(
            direction="minimize", study_name="dst_catboost_tuning"
        )
        ...
        logger.info(
            f"Best CatBoost parameters found (MAE={best_mae:.3f}): {best_params}"
        )
    ...
    def predict(self, X: np.ndarray) -> PredictionResult:
        ...
        confidence_score = (
            np.ones_like(point_estimate) * 0.75
        )  # Higher confidence for CatBoost
    ...
    def load_model(self, path: str):
        ...
        catboost_paths = [
            path.replace(
                ".pth", "_nn_catboost.cbm"
            ),  # New convention: dst_model_nn_catboost.cbm
            path.replace(
                ".pth", "_catboost.cbm"
            ),  # Old convention: dst_model_catboost.cbm
        ]
        ...
        if catboost_path is None:
            raise FileNotFoundError(
                f"CatBoost model not found. Tried: {catboost_paths}"
            )
...
class PositionSpecificHead(nn.Module):
    ...
    def __init__(self, context_dim: int, player_dim: int, hidden_dim: int = 64):
        ...
        self.uncertainty = nn.Sequential(
            nn.Linear(hidden_dim, 16), nn.ReLU(), nn.Linear(16, 2)
        )

    def forward(
        self, game_context: torch.Tensor, player_features: torch.Tensor
...
class EnsembleModel:
    ...
    def __init__(self, neural_model: BaseNeuralModel, position: str):
        ...
        if not HAS_XGBOOST:
            logger.warning(
                "XGBoost not available. Falling back to neural network only."
            )
    ...
    def train(
        ...
        if self.position == "QB":
            xgb_params.update({"max_depth": 7, "learning_rate": 0.05})
            num_rounds = 500
        elif self.position in ["RB", "WR"]:
            xgb_params.update({"max_depth": 6, "learning_rate": 0.08})
            num_rounds = 400
        else:  # TE, DST
            xgb_params.update({"max_depth": 5, "learning_rate": 0.1})
            num_rounds = 300
        ...
        train_r2 = (
            1 - np.sum((y_train - xgb_train_pred) ** 2) / train_ss_tot
            if train_ss_tot > 0
            else 0.0
        )
        val_r2 = (
            1 - np.sum((y_val - xgb_val_pred) ** 2) / val_ss_tot
            if val_ss_tot > 0
            else 0.0
        )
        ...
        return TrainingResult(
            ...
            feature_importance=self.xgb_model.get_score(importance_type="weight")
            if self.xgb_model
            else None,
            train_mae=train_mae,
            val_mae=val_mae,
    ...
    def tune_hyperparameters(
        ...
        if self.position == "QB":
            xgb_params.update({"xgb_max_depth": 7, "xgb_learning_rate": 0.05})
        elif self.position in ["RB", "WR"]:
            xgb_params.update({"xgb_max_depth": 6, "xgb_learning_rate": 0.08})
        else:  # TE, DST
            xgb_params.update({"xgb_max_depth": 5, "xgb_learning_rate": 0.1})
    ...
    def predict(self, X: np.ndarray) -> PredictionResult:
        ...
        uncertainty = (
            nn_result.confidence_score * 0.9
        )  # Slightly more confident with ensemble
    ...
    def load_model(self, path: str, input_size: int = None):
        ...
        if HAS_XGBOOST:
            ...
            try:
                ...
                logger.warning(
                    f"Could not load XGBoost model: {e}. Using neural network only."
                )
                self.xgb_model = None
        else:
...
class CorrelatedFantasyModel(nn.Module):
    ...
    def forward(
        ...
        if len(position_embeddings) > 1:
            ...
            correlated = torch.matmul(
                stacked,
                self.stack_factors[
                    : len(position_embeddings), : len(position_embeddings)
                ],
            )
...
def create_model(
    position: str, config: ModelConfig = None, use_ensemble: bool = False
) -> BaseNeuralModel:
...
def load_trained_model(
    position: str, model_path: str, input_size: int
) -> BaseNeuralModel:
