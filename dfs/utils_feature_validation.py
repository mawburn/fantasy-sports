...
_BINARY_COLS = [
    "is_favorite", "cold_lt40", "hot_gt85", "wind_gt15", "dome",
...
_NUMERIC_ALWAYS = set(_NUMERIC_SOFT_RANGES.keys()) | {
    ...
    "pressure_rate","opp_dvp_pos_allowed","home","rest_days","travel","season_week"
}
...
def enforce_feature_order(df: pd.DataFrame, expected_order: List[str], allow_extra: bool=False) -> pd.DataFrame:
    ...
    if extras and not allow_extra:
        _fail(f"Unexpected extra columns present (set allow_extra=True to ignore): {extras[:20]}")
...
def _assert_binary_col(df: pd.DataFrame, col: str):
    ...
    if not set(np.unique(vals)).issubset(ok):
        _fail(f"Column '{col}' must be binary (0/1). Found values: {sorted(vals.tolist())[:6]}")
...
def _assert_soft_range(df: pd.DataFrame, col: str, lo: float, hi: float):
    ...
    if len(bad) > 0:
        ...
        if frac > 0.01 or bad.abs().max() > max(abs(lo)*1.5, abs(hi)*1.5):
            _fail(f"Column '{col}' outside soft range [{lo},{hi}] for {len(bad)} rows ({frac:.1%}). Example: {bad.iloc[0]}")
...
def validate_and_prepare_features(
    ...
    inj_cols = [c for c in _BINARY_COLS if c.startswith("injury_status_") and c in df.columns]
    if inj_cols:
        sums = df[inj_cols].sum(axis=1)
        if (sums > 1.0 + 1e-6).any():
            _fail(f"Injury status one-hots must be mutually exclusive. Found row-sum > 1 in {int((sums>1.0).sum())} rows.")
    ...
    if len(df) > 50:  # Only check variance for larger datasets
        for col in _NUMERIC_ALWAYS.intersection(df.columns):
            ...
            if len(s) > 0 and float(s.std()) == 0.0:
                _fail(f"Numeric column '{col}' has zero variance; likely a join/feature bug.")
