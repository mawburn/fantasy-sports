...
class DSTModelPipeline:
    ...
    def create_enhanced_features(self) -> pd.DataFrame:
        ...
        df['pressure_expectation'] = df['sacks_l5'] * (1 + df['team_spread'] / 50)  # More pressure when favored
        df['turnover_expectation'] = (df['int_l5'] + df['fr_l5']) * (1 + df['wind_speed'] / 20)  # Wind increases TOs
        df['scoring_defense_strength'] = 20 - df['pa_l5']  # Inverse of points allowed
        df['game_script_advantage'] = df['is_favorite'] * df['is_home'] * (1 + abs(df['team_spread']) / 10)
        ...
        logger.info(f"Created feature dataset with {len(df)} samples and {len(df.columns)} columns")
        logger.info(f"Fantasy points range: [{df['target_fantasy_points'].min():.1f}, {df['target_fantasy_points'].max():.1f}]")
        logger.info(f"Average fantasy points: {df['target_fantasy_points'].mean():.2f} ± {df['target_fantasy_points'].std():.2f}")
        
        return df
    
    def prepare_train_val_split(self, df: pd.DataFrame, val_season: int = 2023, val_week_start: int = 10) -> Tuple:
        """Create time-based train/validation split."""
        
        # Training data: everything before validation period
        train_mask = (df['season'] < val_season) | ((df['season'] == val_season) & (df['week'] < val_week_start))
        val_mask = ~train_mask
        
        feature_cols = [
            'opponent_implied_total', 'game_total', 'team_spread', 'is_home', 'is_favorite',
            'sacks_l5', 'int_l5', 'fr_l5', 'pa_l5', 'td_l5', 'fp_l5',
            'sacks_season', 'fp_season',
            'temperature', 'wind_speed', 'has_precipitation', 'is_dome',
            'pressure_expectation', 'turnover_expectation', 'scoring_defense_strength', 'game_script_advantage'
        ]
        ...
        # Prepare all targets
        targets = {
            'fantasy_points': (df[train_mask]['target_fantasy_points'].values, 
                              df[val_mask]['target_fantasy_points'].values),
            'sacks': (df[train_mask]['target_sacks'].values, 
                     df[val_mask]['target_sacks'].values),
            'turnovers': (df[train_mask]['target_int'].values + df[train_mask]['target_fr'].values,
                         df[val_mask]['target_int'].values + df[val_mask]['target_fr'].values),
            'pa_bucket': (df[train_mask]['target_pa_bucket'].values,
                         df[val_mask]['target_pa_bucket'].values),
            'has_td': (df[train_mask]['target_has_td'].values,
    ...
    def train_direct_model(self, X_train: np.ndarray, X_val: np.ndarray, 
                          y_train: np.ndarray, y_val: np.ndarray) -> CatBoostRegressor:
        ...
        logger.info(f"Direct Model - Train MAE: {train_mae:.3f}, Val MAE: {val_mae:.3f}")
        logger.info(f"Direct Model - Train R²: {train_r2:.3f}, Val R²: {val_r2:.3f}")
        logger.info(f"Direct Model - Val Spearman: {val_corr:.3f}")
        logger.info(f"Direct Model - Prediction range: [{val_pred.min():.1f}, {val_pred.max():.1f}]")
        
        return model
    
    def train_component_models(self, X_train: np.ndarray, X_val: np.ndarray, 
    ...
    def evaluate_models(self, direct_model: Any, component_models: Dict, 
    ...
    def run_full_pipeline(self):
        ...
        direct_model = self.train_direct_model(
            ...
            targets['fantasy_points'][0], targets['fantasy_points'][1]
        )
        
        # Step 4: Train component models
        component_models = self.train_component_models(X_train_norm, X_val_norm, targets)
        
        # Step 5: Evaluate all models
        results = self.evaluate_models(
            direct_model, component_models,
            X_val_norm, targets['fantasy_points'][1]
        )
        ...
        for model_type, metrics in results.items():
            ...
            logger.info(f"  Prediction Range: [{metrics['pred_range'][0]:.1f}, {metrics['pred_range'][1]:.1f}]")
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"BEST MODEL: {best_model.upper()} (MAE: {results[best_model]['mae']:.3f})")
        logger.info(f"{'='*60}")
...
def main():
    ...
    logger.info(f"New Best Model: R² = {best_result['r2']:.3f}, MAE = {best_result['mae']:.3f}")
    logger.info(f"R² Improvement: {(best_result['r2'] - 0.008):.3f} ({(best_result['r2'] / 0.008 - 1) * 100:.0f}% better)")
    logger.info(f"MAE Improvement: {(3.905 - best_result['mae']):.3f} ({(1 - best_result['mae'] / 3.905) * 100:.1f}% better)")
